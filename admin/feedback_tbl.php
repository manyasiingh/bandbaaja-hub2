...
{
	$id = $_GET['did'];
	$q = mysqli_query($connection,"delete from tbl_feedback where feedback_id = $id");
	if($q)
	{
...
FROM
    `tbl_feedback`
    INNER JOIN `tbl_user` 
        ON (`tbl_feedback`.`user_id` = `tbl_user`.`user_id`);") or die(mysqli_error($connection));

while($row = mysqli_fetch_array($query))
{
        echo "<td>{$row['feedback_id']} </td><td>{$row['feedback_details']} </td>  <td>{$row['user_name']} </td>";
