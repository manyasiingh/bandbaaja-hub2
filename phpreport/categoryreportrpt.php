...
// Create page object
if (!isset($categoryreport_rpt))
	$categoryreport_rpt = new categoryreport_rpt();
if (isset($Page))
	$OldPage = $Page;
$Page = &$categoryreport_rpt;
...
<div id="ew-center" class="<?php echo $categoryreport_rpt->CenterContentClass ?>">
<?php } ?>
...
<!-- Report grid (begin) -->
<?php if ($Page->Export <> "pdf") { ?>
<div id="gmp_categoryreport" class="<?php if (IsResponsiveLayout()) { echo "table-responsive "; } ?>ew-grid-middle-panel">
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="category_id"><div class="categoryreport_category_id"><span class="ew-table-header-caption"><?php echo $Page->category_id->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="category_id">
<?php if ($Page->sortUrl($Page->category_id) == "") { ?>
		<div class="ew-table-header-btn categoryreport_category_id">
			<span class="ew-table-header-caption"><?php echo $Page->category_id->caption() ?></span>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer categoryreport_category_id" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->category_id) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->category_id->caption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="category_name"><div class="categoryreport_category_name"><span class="ew-table-header-caption"><?php echo $Page->category_name->caption() ?></span></div></td>
<?php } else { ?>
	<td data-field="category_name">
<?php if ($Page->sortUrl($Page->category_name) == "") { ?>
		<div class="ew-table-header-btn categoryreport_category_name">
			<span class="ew-table-header-caption"><?php echo $Page->category_name->caption() ?></span>
		</div>
<?php } else { ?>
		<div class="ew-table-header-btn ew-pointer categoryreport_category_name" onclick="ew.sort(event,'<?php echo $Page->sortUrl($Page->category_name) ?>',2);">
			<span class="ew-table-header-caption"><?php echo $Page->category_name->caption() ?></span>
...
<?php include "categoryreport_pager.php" ?>
